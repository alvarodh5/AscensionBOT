        ...
        static internal void Initialize(string databaseType, string parConnectionString)
        {
            switch (databaseType.ToLower())
            {
                ...
                case "mssql":
                    ...
                    break;
                case "none":
                    // No database. Used for a simple grind bot that doesn't need
                    // vendors/hotspots/blacklist persistence. Avoids the native
                    // SQLite.Interop.dll entirely (which is painful when injected).
                    databaseWrapper = new NullRepository();
                    break;
                default:
