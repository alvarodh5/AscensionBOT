...
namespace AscensionBot.AI.SharedStates
{
    public class EquipArmorState : IBotState
    {
        ...
        public void Update()
        {
            ...
            // Classless / custom classes (e.g. Project Ascension, class byte outside 1-11)
            // aren't in the armor-type map. Skip auto-equip instead of crashing with
            // KeyNotFoundException, and continue the normal flow (rest -> grind).
            if (!desiredArmorTypes.ContainsKey(player.Class))
            {
                botStates.Pop();
