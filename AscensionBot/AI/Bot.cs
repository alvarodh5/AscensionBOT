...
namespace AscensionBot.AI
{
    public abstract class Bot
    {
        ...
        // While recovering from death or waiting out Resurrection Sickness, standing still is
        // expected — the stuck killswitches must NOT fire (Stop() also halts AntiAfk -> disconnect).
        static bool HasResSickness(LocalPlayer p) =>
            p.HasDebuff("Resurrection Sickness") || p.HasBuff("Resurrection Sickness") ||
            p.HasDebuff("Enfermedad por resurrección") || p.HasBuff("Enfermedad por resurrección");

        public void Stop()
        {
            // Final Telegram session report (Caster only; no-op otherwise). Runs before we tear
            // down state so the report reflects the full session.
            SessionStats.StopAndReport("bot detenido");
            ...
            while (running)
            {
                try
                {
                    ...
                    ThreadSynchronizer.RunOnMainThread(() =>
                    {
                        ...
                        {
                            TelegramClientWrapper.TeleportAlert(player.Name);
                            Stop();
                            return;
                        }
                        ...
                        // (skip while recovering from death — standing at the spirit healer /
                        // ghost-walking is legitimately slow and must not trip the killswitch,
                        // because Stop() also halts AntiAfk and gets us disconnected).
                        if (Environment.TickCount - currentStateStartTime > 300000 && currentState != typeof(TravelState) && container.BotSettings.UseStuckInStateKillswitch && !retrievingCorpse && !HasResSickness(player))
                        {
                            ...
                            LogToFile(msg);
                            TelegramClientWrapper.SendMessage(msg);
                            Stop();
                            return;
                        }
                        ...
                        if (Environment.TickCount - currentPositionStartTime > 300000 && container.BotSettings.UseStuckInPositionKillswitch && !retrievingCorpse && !HasResSickness(player))
                        {
        ...
        async void StartInternal(IDependencyContainer container)
        {
            while (running)
            {
                try
                {
                    ...
                    ThreadSynchronizer.RunOnMainThread(() =>
                    {
                        // Session stats + Telegram report + disconnect detection (Caster only;
                        // no-op for other profiles since they don't start SessionStats).
                        if (SessionStats.Running)
                            SessionStats.Tick(botStates.Count > 0 ? botStates.Peek()?.GetType().Name : null);

                        // If we are disconnected, try logging in. (Skip auto-reconnect while the
                        // Caster session is running — it detects disconnects and stops instead.)
                        if (!SessionStats.Running && LoginState.ShouldLogin() &&
                        ...
                        if (player.Level > currentLevel)
                        {
                            ...
                            TelegramClientWrapper.SendMessage($"Ding! {player.Name} is now level {player.Level}!");
                            if (SessionStats.Running)
                                SessionStats.Report("⬆️ Subida de nivel");
                        }
                        ...
                        if ((player.Health <= 0 || player.InGhostForm) && !retrievingCorpse)
                        {
                            ...
                            // Go straight to the Spirit Healer instead of a corpse run: without
                            // mmaps we can't navigate to the corpse, and the corpse-position
                            // offset appears unreliable on Ascension (ghost just walked off in a
                            // fixed wrong direction). The Spirit Healer spawns right next to the
                            // ghost and is reliable. Resurrection Sickness (if any) is waited out
                            // in GrindState. Release spirit first, then resurrect at the healer.
                        ...
                        // This is a workaround to prevent that from causing us to get caught in a loop.
                        // We default to a durability value of 100 for items that are null because 100 will register them as not needing repaired.
                        if (currentHotspot != null && (mainhandDurability <= 20 && mainhandDurability > -1 || (offhandDurability <= 20 && offhandDurability > -1)) && currentHotspot.RepairVendor != null && !container.RunningErrands)
                        {
                            ShapeshiftToHumanForm(container);
                            PopStackToBaseState();
                        ...
                        // (currentHotspot != null guard: with no database/hotspot there is no
                        // vendor to sell to, so skip the errand instead of NRE'ing. Just keep killing.)
                        if (currentHotspot != null &&
                            Inventory.CountFreeSlots(false) == 0 &&
                            (
