    ...
    public class DependencyContainer : IDependencyContainer
    {
        ...
        public WoWUnit FindClosestTarget()
        {
            ...
            var potentialTargetsList = ObjectManager.Units
                ...
                // exclude units we can't attack. UnitFlags is a bitfield, so test with HasFlag
                // (the old `!= UNIT_FLAG_NON_ATTACKABLE` failed whenever any other flag was set).
            ...
            try
            {
                ...
                foreach (var player in PlayerTrackers)
                {
                    ...
                    if (targetWarningTriggered)
                    {
                        ...
                        TelegramClientWrapper.SendMessage($"{player.Key} has been targeting {ObjectManager.Player.Name} for over {BotSettings.TargetingWarningTimer}ms.");
                    }
                    ...
                    if (proximityWarningTriggered)
                    {
                        ...
                        TelegramClientWrapper.SendMessage($"{player.Key} has been in range of {ObjectManager.Player.Name} for over {BotSettings.ProximityWarningTimer}ms.");
                    }
                    ...
                    {
                        TelegramClientWrapper.SendMessage($"{player.Key} has been targeting {ObjectManager.Player.Name} for over {BotSettings.TargetingStopTimer}ms. Stopping.");
                    ...
                    {
                        TelegramClientWrapper.SendMessage($"{player.Key} has been in range of {ObjectManager.Player.Name} for over {BotSettings.ProximityStopTimer}ms. Stopping.");
                        Console.Beep(1000, 250);
                        stopBot = true;
                    }
